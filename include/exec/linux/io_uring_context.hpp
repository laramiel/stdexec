...
namespace exec {
  namespace __io_uring {
    ...
    inline void __throw_error_code_if(bool __cond, int __ec) {
      if (__cond) {
        STDEXEC_INTERNAL_THROW(std::system_error(__ec, std::system_category()));
      }
    }
     ...
     public:
      ...
      void reset() {
        if (__is_running_.load(std::memory_order_relaxed) || __n_total_submitted_ > 0) {
          STDEXEC_INTERNAL_THROW(std::runtime_error("exec::io_uring_context::reset() called on a running context"));
        }
      ...
      void run_until_stopped() {
        ...
        if (!__is_running_.compare_exchange_strong(
          ...
          STDEXEC_INTERNAL_THROW(std::runtime_error("exec::io_uring_context::run() called on a running context"));
        } else {
